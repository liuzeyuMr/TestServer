...
int Server_select(){
    ...
    while(1){
        ...
        if (FD_ISSET(listenfd,&rset)){ //have a client apply to connect ,get the connect and add to the allset
            ...
            FD_SET(connfd,&allset); //将新链接的FD加入集合中
            if (maxfd < connfd)
                maxfd= connfd;
            if (maxi < i){
                maxi = i;
...
int Server_poll(){
    ...
    maxi=0;
    for (i=1;i<FD_SETSIZE;i++) //从1开始将client.fd赋值-1
        client[i].fd = -1;
...
int Server_epoll()
{
    ...
    while (1){
        ...
        int n;
        for (n=0;n<nready;n++){
            if (!(evts[n].events & EPOLLIN)) // 这里只判断是否有读事件满足
                continue;
            if (evts[n].data.fd == listenfd){ // 进行listenfd的比较
                clientlen = sizeof(client); //有新的链接请求加入
                ...
                evt.events = EPOLLIN | EPOLLET;//读事件 和 边沿触发
                evt.data.fd =connfd;
                int res;
                ...
                socketfd = evts[n].data.fd; //有新的数据可读
