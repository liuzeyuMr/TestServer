//
// Created by liuzeyu on 2021/3/28.
//
...
void eventadd(int efd, int events, struct myevent_s *ev)
{
    ...
    if(epoll_ctl(efd, op, ev->fd, &epv) < 0) // 添加一个节点
        ...
        printf("[%s][%d]event add OK [fd=%d],events[%0X]\n",__FUNCTION__ ,__LINE__, ev->fd, events);
    return;
}
...
void acceptconn(int lfd,int events,void *arg)
{
    ...
    printf("[%s][%d]new connect[%s:%d],[time:%ld],pos[%d] \n",__FUNCTION__ ,__LINE__,inet_ntoa(cin.sin_addr), ntohs(cin.sin_port), g_events[i].last_active, i);
    return;
}
...
void senddata(int fd, int events, void *arg)
{
    ...
    len = send(fd, ev->buf, ev->len, 0);    //将数据小写转大写然后直接将数据回射给客户端
...
int main()
{
    ...
    printf("[%s][%d]server running:port[%d]\n",__FUNCTION__ ,__LINE__, port);
    ...
    while(1)
    {
        ...
        for(i = 0; i < nfd; i++)
        {
            ...
            //如果监听的是写事件，并返回的是写事件 这里读事件和写事件都是同意回调函数
